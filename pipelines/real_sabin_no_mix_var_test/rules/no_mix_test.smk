rule map_to_ref:
    ...
    output:
        "test/{iteration}/no_mix/map_ref/distance_{distance}_real.bam"
    shell:
...
rule index_map_to_ref:
    input:
        "test/{iteration}/no_mix/map_ref/distance_{distance}_real.bam"
    output:
        "test/{iteration}/no_mix/map_ref/distance_{distance}_real.bam.bai"
    shell:
...
rule map_to_cns:
    ...
    output:
        "test/{iteration}/no_mix/map_cns/distance_{distance}_real.bam"
    shell:
        "minimap2 -ax map-ont {input.reference} {input.reads} | samtools sort -O bam -o {output}"
...
rule index_map_to_cns:
    input:
        "test/{iteration}/no_mix/map_cns/distance_{distance}_real.bam"
    output:
        "test/{iteration}/no_mix/map_cns/distance_{distance}_real.bam.bai"
    shell:
        "samtools index {input}"
...
rule polish_call:
    input:
        ...
        map = "test/{iteration}/no_mix/map_ref/distance_{distance}_real.bam",
        index = "test/{iteration}/no_mix/map_ref/distance_{distance}_real.bam.bai"
    params:
        dir = "test/{iteration}/no_mix/calls_cns/distance_{distance}_real"
    output:
        "test/{iteration}/no_mix/calls_cns/distance_{distance}_real/round_1_phased.vcf"
    shell:
...
rule call:
    ...
    output:
        "test/{iteration}/no_mix/calls_ref/distance_{distance}_real/round_1_phased.vcf"
    shell:
...
rule make_fasta_sabin_vs_ref_vs_cns:
    ...
    output:
        three = "test/{iteration}/alignments/distance_{distance}_real.fasta",
...
rule align_sabin_ref_cns:
    input:
        "test/{iteration}/alignments/distance_{distance}_real.fasta"
    output:
        "test/{iteration}/alignments/distance_{distance}_real.aln.fasta"
    shell:
        "mafft {input} > {output}"
